<script lang="ts">
	...
	type CardVariant = 'plain' | 'standard' | 'resource' | 'person' | 'count' | 'quote';
	type CardTone = 'teal' | 'gold' | 'navy' | 'violet';
	type CardPadding = 'compact' | 'default' | 'spacious';
	type CardHeadingLevel = 'h2' | 'h3' | 'h4';
	type CardElement = 'article' | 'aside' | 'div' | 'figure' | 'section' | 'li';
	...
	const isLinkDisabled = $derived(!linkHref || linkHref === '#');
	const isCtaDisabled = $derived(Boolean(cta && (!cta.href || cta.href === '#')));
</script>

<svelte:element this={as} class={cardClass}>
	{#if variant === 'count'}
	...
	<div class="card-main">
		...
		{#if hasLink}
			{#if isLinkDisabled}
				<span class="card-link disabled" aria-disabled="true">{linkLabel}</span>
			{:else}
				<a class="card-link" href={linkHref}>{linkLabel}</a>
			{/if}
		{/if}

		{#if cta}
			<div class="cta">
				<Button href={cta.href} label={cta.label} variant={cta.variant} disabled={isCtaDisabled} />
			</div>
		{/if}
...
<style>
	...
	.resource {
		border-color: color-mix(in srgb, var(--card-accent) 54%, var(--ec-border-soft));
	}
