<script lang="ts">
	...
	import type { ResourcesPressPage, SiteChrome } from '$lib/content/types';

	type Props = {
		data: {
			page: ResourcesPressPage;
			chrome: SiteChrome;
		};
	};
...
<svelte:head>
	...
	<meta
		name="description"
		content={page.hero.description ?? 'Press releases and charters from Education Data Unlimited.'}
	/>
</svelte:head>

<PrimaryNav links={chrome.primaryNav} footerColumns={chrome.footerColumns} activeSection={page.activeSection} activeSubSection="Press & charter" />
<SubNav crumb="Resources" crumbHref="/resources" links={page.subNav} active="Press & charter" />

<main>
	<Hero content={page.hero} background="teal" icon="resource" />

	<section class="section section-padded" aria-labelledby="press-heading">
		<Container>
			<div class="section-header">
				<p class="eyebrow">Categories</p>
				<h2 id="press-heading">Press releases and charters</h2>
			</div>

			<div class="content-layout">
				<CategorySelector
					categories={page.categories}
					label="Press and charter categories"
				...
				<div class="document-list">
					{#if filteredItems.length}
						...
						<Pagination
							...
							bind:currentPage
							bind:itemsPerPage
							label="Press and charter pagination"
						/>
					{:else}
						<div class="empty-state" role="status">
							<h3>No documents available</h3>
							<p>There are no press or charter documents available for {selectedCategory} yet.</p>
						</div>
					{/if}
				</div>
			</div>
