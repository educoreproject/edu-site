...
<svelte:head>
	...
	<meta
		name="description"
		content={page.hero.description ?? 'Newsletter archive from Education Data Unlimited.'}
	/>
</svelte:head>
...
<SubNav crumb="Resources" links={page.subNav} active="Newsletter" />

<main>
	<Hero content={page.hero} background="teal" icon="resource" />

	<section class="section section-padded" aria-labelledby="newsletter-heading">
		<Container>
			<div class="section-header">
				<p class="eyebrow">Categories</p>
				<h2 id="newsletter-heading">Newsletter archive</h2>
			</div>

			<div class="content-layout">
				<CategorySelector
					categories={page.categories}
					label="Newsletter archive months"
				...
				<div class="document-list">
					{#if filteredItems.length}
						...
						<Pagination
							...
							bind:currentPage
							bind:itemsPerPage
							label="Newsletter archive pagination"
						/>
					{:else}
						<div class="empty-state" role="status">
							<h3>No newsletters available</h3>
							<p>There are no newsletters available for {selectedCategory} yet.</p>
						</div>
					{/if}
				</div>
			</div>
